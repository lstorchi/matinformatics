...
do
  ...
  if [ $END -ge $MAXNUM ]; then
    python3 $PROGNAME -S -f feature_rmse.csv -n 20 -k newadata.pkl -r "$START:$MAXNUM" -F $MAXNUM 1> out_$i 2> err_$i &
    echo "Process " $i " will start from " $START " to " $MAXNUM " of " $MAXNUM
    echo ""
    echo "Next start from " $MAXNUM " and procs " $(( $i + 1))
    exit
  fi
  python3 $PROGNAME -S -f feature_rmse.csv -n 20 -k newadata.pkl -r "$START:$END" -F $MAXNUM 1> out_$i 2> err_$i &
